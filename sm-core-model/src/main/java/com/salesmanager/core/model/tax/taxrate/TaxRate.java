...
@Table(name = "TAX_RATE", schema = SALESMANAGER_SCHEMA, uniqueConstraints = {
...
public class TaxRate extends SalesManagerEntity<Long, TaxRate> implements Auditable {
    ...
    @Valid
    @OneToMany(mappedBy = "taxRate", cascade = ALL, fetch = LAZY)
    private List<TaxRateDescription> descriptions = new ArrayList<TaxRateDescription>();

    @ManyToOne(fetch = LAZY, targetEntity = Country.class)
