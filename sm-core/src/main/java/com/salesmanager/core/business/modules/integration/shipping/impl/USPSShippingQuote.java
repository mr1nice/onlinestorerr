...
import static com.salesmanager.core.business.utils.DataUtils.*;
import static com.salesmanager.core.constants.MeasureUnit.IN;
import static com.salesmanager.core.constants.MeasureUnit.LB;
...
public class USPSShippingQuote implements ShippingQuoteModule {
    ...
    private static final Logger LOGGER = getLogger(USPSShippingQuote.class);
    ...
    public void validateModuleConfiguration(
        ...
        if (keys == null || isBlank(keys.get("account"))) {
...
/*		Country country = delivery.getCountry();
        ...
        if (!language.equals(FRENCH.getLanguage())
                && !language.equals(ENGLISH.getLanguage())) {
...
/*		if (!store.getCurrency().equals(Constants.CURRENCY_CODE_CAD)) {
        ...
        try {
            ...
            for (PackageDetails detail : packages) {
                ...
                // need size in inch
                double w = getMeasure(detail.getShippingWidth(),
                        store, IN.name());
                double h = getMeasure(detail.getShippingHeight(),
                        store, IN.name());
                double l = getMeasure(detail.getShippingLength(),
                        store, IN.name());
                ...
                double p = getWeight(detail.getShippingWeight(), store, LB.name());
...
/*			BigDecimal convertedOrderTotal = CurrencyUtil.convertToCurrency(
            ...
            String ouncesString = valueOf(totalP - pounds);
            ...
            String encoded = encode(xmlbuffer.toString());
