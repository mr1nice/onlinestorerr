...
import static com.salesmanager.core.business.utils.DataUtils.trimPostalCode;
...
public class UPSShippingQuote implements ShippingQuoteModule {
    ...
    private static final Logger LOGGER = getLogger(UPSShippingQuote.class);
    ...
    public void validateModuleConfiguration(
        ...
        if (keys == null || isBlank(keys.get("accessKey"))) {
            errorFields = new ArrayList<String>();
            errorFields.add("accessKey");
        }

        if (keys == null || isBlank(keys.get("userId"))) {
            errorFields = new ArrayList<String>();
...
/*		List<String> services = options.get("services");
    ...
    public List<ShippingOption> getShippingQuotes(
        ...
        notNull(configuration, "IntegrationConfiguration must not be null for USPS shipping module");
        ...
        try {
            ...
            StringEntity entity = new StringEntity(xmlbuffer.toString(), APPLICATION_ATOM_XML);
...
/*			if (result != 200) {
            ...
            if (!isBlank(parsed.getErrorCode())) {
