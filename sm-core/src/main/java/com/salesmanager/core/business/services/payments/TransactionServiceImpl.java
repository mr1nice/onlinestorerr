...
public class TransactionServiceImpl  extends SalesManagerEntityServiceImpl<Long, Transaction> implements TransactionService {
	...
	public Transaction getRefundableTransaction(Order order)
		...
		for(Transaction transaction : transactions) {
            if (transaction.getTransactionType().name().equals(AUTHORIZECAPTURE.name())) {
                finalTransactions.put(AUTHORIZECAPTURE.name(), transaction);
                continue;
            }
            if (transaction.getTransactionType().name().equals(CAPTURE.name())) {
                finalTransactions.put(CAPTURE.name(), transaction);
                continue;
            }
            if (transaction.getTransactionType().name().equals(REFUND.name())) {
                Transaction previousRefund = finalTransactions.get(REFUND.name());
                if (previousRefund != null) {
                    ...
                    if (previousDate.before(currentDate)) {
                        finalTransactions.put(REFUND.name(), transaction);
                        continue;
                    }
                } else {
                    finalTransactions.put(REFUND.name(), transaction);
