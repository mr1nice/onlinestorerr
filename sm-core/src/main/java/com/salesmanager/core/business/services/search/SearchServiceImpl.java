...
import static com.salesmanager.core.business.constants.Constants.FALSE;
...
public class SearchServiceImpl implements com.salesmanager.core.business.services.search.SearchService {
	...
	public void index(MerchantStore store, Product product)
        ...
        if (configuration.getProperty(INDEX_PRODUCTS) == null || configuration.getProperty(INDEX_PRODUCTS).equals(FALSE)) {
            return;
        }
        ...
        for (ProductDescription description : descriptions) {
            ...
            if (!isBlank(description.getMetatagKeywords())) {
                String[] tags = description.getMetatagKeywords().split(",");
                @SuppressWarnings("unchecked")
                List<String> tagsList = new ArrayList(asList(tags));
	...
	public SearchKeywords searchForKeywords(String collectionName, String jsonString, int entriesCount) throws ServiceException {
        ...
        try {
            ...
            keywords.setKeywords(asList(response.getInlineSearchList()));
			...
			if(response != null) {
				...
				if(!CollectionUtils.isEmpty(hits)) {
					for(SearchHit hit : hits) {
                        ...
                        indexProduct.setPrice(valueOf(((String) metaEntries.get("price"))));
