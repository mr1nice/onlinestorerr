...
import static com.salesmanager.core.model.shipping.ShippingOptionPriceType.*;
import static com.salesmanager.core.model.shipping.ShippingPackageType.BOX;
import static com.salesmanager.core.model.shipping.ShippingPackageType.ITEM;
import static com.salesmanager.core.model.shipping.ShippingQuote.*;
import static com.salesmanager.core.model.shipping.ShippingType.INTERNATIONAL;
import static com.salesmanager.core.model.shipping.ShippingType.NATIONAL;
import static com.shopizer.search.utils.DateUtil.formatDate;
...
public class ShippingServiceImpl implements ShippingService {
	...
	public ShippingQuote getShippingQuote(Long shoppingCartId, MerchantStore store, Delivery delivery, List<ShippingProduct> products, Language language) throws ServiceException {
        ...
        notNull(delivery, "Delivery must not be null");
        notEmpty(products, "products must not be empty");
        notNull(language, "Language must not be null");
        ...
        try {
            ...
            if (isBlank(delivery.getPostalCode())) {
                ...
                shippingQuote.setShippingReturnCode(NO_POSTAL_CODE);
            }
            ...
            notNull(shipCountry);
            notNull(store.getCountry());

            if (shippingType.name().equals(NATIONAL.name())) {
                if (!shipCountry.getIsoCode().equals(store.getCountry().getIsoCode())) {
                    shippingQuote.setShippingReturnCode(NO_SHIPPING_TO_SELECTED_COUNTRY + " " + shipCountry.getIsoCode());
                    return shippingQuote;
                }
            } else if (shippingType.name().equals(INTERNATIONAL.name())) {
            ...
            if (modules == null) {
                shippingQuote.setShippingReturnCode(NO_SHIPPING_MODULE_CONFIGURED);
                return shippingQuote;
            }
            ...
            if (shippingConfiguration.isFreeShippingEnabled()) {
                ...
                if (freeShippingAmount != null) {
                    if (orderTotal.doubleValue() > freeShippingAmount.doubleValue()) {
                        if (shippingConfiguration.getFreeShippingType() == NATIONAL) {
            ...
            if (!isEmpty(shippingModulePreProcessors)) {
            ...
            try {
                ...
                shippingQuote.setShippingReturnCode(ERROR);
                return shippingQuote;
            }

            if (shippingOptions == null && !isBlank(delivery.getPostalCode())) {
                shippingQuote.setShippingReturnCode(NO_SHIPPING_TO_SELECTED_COUNTRY);
            }
            ...
            if (shippingOptions != null) {
                ...
                for (ShippingOption option : shippingOptions) {
                    ...
                    if (isBlank(option.getOptionName())) {
                    ...
                    if (shippingOptionPriceType.name().equals(HIGHEST.name())) {
                    ...
                    if (shippingOptionPriceType.name().equals(LEAST.name())) {
                    ...
                    if (shippingOptionPriceType.name().equals(ALL.name())) {
                ...
                if (selectedOption != null && !shippingOptionPriceType.name().equals(ALL.name())) {
            ...
            if (!isEmpty(shippingModulePostProcessors)) {
            ...
            for (ShippingOption option : finalShippingOptions) {
                ...
                if (!isBlank(option.getEstimatedNumberOfDays())) {
                    try {
                ...
                if (freeShipping) {
                    ...
                    if (!isBlank(option.getOptionDeliveryDate())) {
                        try {
                            q.setOptionDeliveryDate(formatDate(option.getOptionDeliveryDate()));
                        } catch (Exception e) {
	...
	public List<PackageDetails> getPackagesDetails(
        ...
        ShippingPackageType shippingPackageType = ITEM;
        if (shippingConfiguration != null) {
            shippingPackageType = shippingConfiguration.getShippingPackageType();
        }

        if (shippingPackageType.name().equals(BOX.name())) {
