...
public class CmsStaticContentFileManagerImpl implements FilePut, FileGet, FileRemove {
    ...
    private static final Logger LOGGER = getLogger(CmsStaticContentFileManagerImpl.class);
...
/*        if ( cacheManager.getTreeCache() == null )
        ...
        try {
            ...
            //file path
            nodePath.append(SLASH).append(inputStaticContentData.getFileContentType()).append(SLASH);
            ...
            copy(inputStaticContentData.getFile(), path, REPLACE_EXISTING);
...
/*        if ( cacheManager.getTreeCache() == null )
        ...
        try {
            ...
            for (final InputContentFile inputStaticContentData : inputStaticContentDataList) {
                ...
                //file creation
                nodePath.append(SLASH).append(inputStaticContentData.getFileName());
    ...
    public List<String> getFileNames(final String merchantStoreCode, final FileContentType staticContentType)
        ...
        try {
            ...
            if (exists(path)) {
                ...
                DirectoryStream<Path> directoryStream = newDirectoryStream(path);
                for (Path dirPath : directoryStream) {

                    String fileName = dirPath.getFileName().toString();

                    if (staticContentType.name().equals(IMAGE.name())) {
                        //File f = new File(fileName);
                        String mimetype = guessContentTypeFromName(fileName);
