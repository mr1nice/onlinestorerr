...
import static com.salesmanager.core.model.tax.TaxBasisCalculation.*;
...
public class TaxServiceImpl 
	...
	public List<TaxItem> calculateTax(OrderSummary orderSummary, Customer customer, MerchantStore store, Language language) throws ServiceException {
        ...
        if (taxConfiguration == null) {
            ...
            taxConfiguration.setTaxBasisCalculation(SHIPPINGADDRESS);
        }
        ...
        if (taxBasisCalculation.name().equals(SHIPPINGADDRESS)) {
        ...
        } else if (taxBasisCalculation.name().equals(BILLINGADDRESS)) {
        ...
        } else if (taxBasisCalculation.name().equals(STOREADDRESS)) {
        ...
        if (!taxConfiguration.isCollectTaxIfDifferentProvinceOfStoreCountry()) {
            ...
            if (!isBlank(stateProvince)) {
                if (store.getZone() != null) {
                    if (!store.getZone().getName().equals(stateProvince)) {
                        return null;
                    }
                } else if (!isBlank(store.getStorestateprovince())) {
        ...
        if (zone == null && isBlank(stateProvince)) {
            return null;
        }
        ...
        for (Long taxClassId : taxClassAmountMap.keySet()) {
            ...
            if (!isBlank(stateProvince) && zone == null) {
            ...
            totalTaxedItemValue.setScale(2, HALF_UP);
            ...
            for (TaxRate taxRate : taxRates) {
                ...
                double roundedValue = new BigDecimal(value).setScale(2, HALF_UP).doubleValue();
                taxedItemValue = new BigDecimal(roundedValue).setScale(2, HALF_UP);
