    ...
    public int doStartTagInternal() throws JspException {
        try {
            ...
            if (merchantStore == null) {
                merchantStore = (MerchantStore) request.getAttribute(ADMIN_STORE);
            }
            ...
            if (!isBlank(this.getProductDescription().getSeUrl())) {
                productPath.append(PRODUCT_URI).append("/");
                productPath.append(this.getProductDescription().getSeUrl());
            } else {
                productPath.append(PRODUCT_ID_URI).append("/");
                productPath.append(this.getProductDescription().getProduct().getSku());
            }

            productPath.append(URL_EXTENSION);
