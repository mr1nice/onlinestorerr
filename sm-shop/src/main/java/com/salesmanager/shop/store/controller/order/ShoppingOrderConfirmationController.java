...
import static com.salesmanager.shop.store.controller.ControllerConstants.Tiles.Checkout.confirmation;
...
public class ShoppingOrderConfirmationController extends AbstractController {
    ...
    public String displayConfirmation(Model model, HttpServletRequest request, HttpServletResponse response, Locale locale) throws Exception {
        ...
        Long orderId = super.getSessionAttribute(ORDER_ID, request);
        if (orderId == null) {
            return new StringBuilder().append("redirect:").append(SHOP_URI).toString();
        }
        ...
        if (super.getSessionAttribute(ORDER_ID_TOKEN, request) != null) {
            //set this unique token for performing unique operations in the confirmation
            model.addAttribute("confirmation", "confirmation");
        }

        //remove unique token
        super.removeAttribute(ORDER_ID_TOKEN, request);
        ...
        String[] orderIdParams = {valueOf(order.getId())};
        ...
        /** template **/
        StringBuilder template = new StringBuilder().append(confirmation).append(".").append(store.getStoreTemplate());
