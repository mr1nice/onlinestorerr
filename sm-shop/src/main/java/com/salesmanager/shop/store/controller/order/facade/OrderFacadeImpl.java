...
import static com.salesmanager.core.model.payments.PaymentType.*;
...
public class OrderFacadeImpl implements OrderFacade {
	...
	private Order processOrderModel(ShopOrder order, Customer customer, Transaction transaction, MerchantStore store,
		...
		try {
            ...
            modelOrder.setPaymentType(valueOf(order.getPaymentMethodType()));
            modelOrder.setShippingModuleCode(order.getShippingModule());
            modelOrder.setCustomerAgreement(order.isCustomerAgreed());
            modelOrder.setLocale(getLocale(store));
            ...
            if (!isBlank(order.getShippingModule())) {
                modelOrder.setShippingModuleCode(order.getShippingModule());
            }

            String paymentType = order.getPaymentMethodType();
            Payment payment = new Payment();
            payment.setPaymentType(valueOf(paymentType));
            ...
            if (CREDITCARD.name().equals(paymentType)) {
                ...
                if (AMEX.name().equalsIgnoreCase(cardType)) {
                    creditCardType = AMEX;
                } else if (VISA.name().equalsIgnoreCase(cardType)) {
                    creditCardType = VISA;
                } else if (MASTERCARD.name().equalsIgnoreCase(cardType)) {
                    creditCardType = MASTERCARD;
                } else if (DINERS.name().equalsIgnoreCase(cardType)) {
                    creditCardType = DINERS;
                } else if (DISCOVERY.name().equalsIgnoreCase(cardType)) {
                    creditCardType = DISCOVERY;
                }
                ...
                if (creditCardType != null) {
                    ...
                    String maskedNumber = maskCardNumber(order.getPayment().get("creditcard_card_number"));
            ...
            if (PAYPAL.name().equals(paymentType)) {
                if (transaction == null) {
	...
	public void validateOrder(ShopOrder order, BindingResult bindingResult, Map<String,String> messagesResult, MerchantStore store,  Locale locale) throws ServiceException {
		...
		try {
            if (isBlank(order.getCustomer().getBilling().getFirstName())) {
            ...
            if (isBlank(order.getCustomer().getBilling().getLastName())) {
            ...
            if (isBlank(order.getCustomer().getEmailAddress())) {
            ...
            if (isBlank(order.getCustomer().getBilling().getAddress())) {
            ...
            if (isBlank(order.getCustomer().getBilling().getCity())) {
            ...
            if (isBlank(order.getCustomer().getBilling().getCountry())) {
            ...
            if (isBlank(order.getCustomer().getBilling().getZone()) && isBlank(order.getCustomer().getBilling().getStateProvince())) {
            ...
            if (isBlank(order.getCustomer().getBilling().getPhone())) {
            ...
            if (isBlank(order.getCustomer().getBilling().getPostalCode())) {
            ...
            if (!order.isShipToBillingAdress()) {
                ...
                if (isBlank(order.getCustomer().getDelivery().getFirstName())) {
                ...
                if (isBlank(order.getCustomer().getDelivery().getLastName())) {
                ...
                if (isBlank(order.getCustomer().getDelivery().getAddress())) {
                ...
                if (isBlank(order.getCustomer().getDelivery().getCity())) {
                ...
                if (isBlank(order.getCustomer().getDelivery().getCountry())) {
                ...
                if (isBlank(order.getCustomer().getDelivery().getZone()) && isBlank(order.getCustomer().getDelivery().getStateProvince())) {
                ...
                if (isBlank(order.getCustomer().getDelivery().getPostalCode())) {
            ...
            if (paymentType == null) {
                ServiceException serviceException = new ServiceException(EXCEPTION_VALIDATION, "payment.required");
                throw serviceException;
            }
            if (shippingService.requiresShipping(order.getShoppingCartItems(), store) && order.getSelectedShippingOption() == null) {
                ServiceException serviceException = new ServiceException(EXCEPTION_VALIDATION, "shipping.required");
                throw serviceException;
            }
            if (CREDITCARD.name().equals(paymentType) && "true".equals(coreConfiguration.getProperty("VALIDATE_CREDIT_CARD"))) {
                ...
                if (isBlank(cco) || isBlank(cvv) || isBlank(ccn) ||
                        isBlank(ccm) || isBlank(ccd)) {
                ...
                if (cardType.equalsIgnoreCase(AMEX.name())) {
                    creditCardType = AMEX;
                } else if (cardType.equalsIgnoreCase(VISA.name())) {
                    creditCardType = VISA;
                } else if (cardType.equalsIgnoreCase(MASTERCARD.name())) {
                    creditCardType = MASTERCARD;
                } else if (cardType.equalsIgnoreCase(DINERS.name())) {
                    creditCardType = DINERS;
                } else if (cardType.equalsIgnoreCase(DISCOVERY.name())) {
                    creditCardType = DISCOVERY;
                }

                if (creditCardType == null) {
                    ServiceException serviceException = new ServiceException(EXCEPTION_VALIDATION, "cc.type");
                    throw serviceException;
                }
	...
	public ShippingQuote getShippingQuote(Customer customer, ShoppingCart cart, MerchantStore store, Language language)
        ...
        notNull(cart, "cart cannot be null");
        List<ShippingProduct> shippingProducts = shoppingCartService.createShippingProduct(cart);
        ...
        Delivery delivery = new Delivery();
        if (customer.getDelivery() == null || isBlank(customer.getDelivery().getPostalCode())) {
            Billing billing = customer.getBilling();
            delivery.setAddress(billing.getAddress());
            delivery.setCity(billing.getCity());
            delivery.setCompany(billing.getCompany());
