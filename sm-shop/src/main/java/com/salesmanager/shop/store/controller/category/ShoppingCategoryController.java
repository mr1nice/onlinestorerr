...
import static com.salesmanager.shop.store.controller.ControllerConstants.Tiles.Category.category;
import static com.salesmanager.shop.store.model.filter.QueryFilterType.BRAND;
import static com.salesmanager.shop.utils.PageBuilderUtils.build404;
import static com.salesmanager.shop.utils.PageBuilderUtils.buildHomePage;
...
public class ShoppingCategoryController {
    ...
    private static final Logger LOGGER = getLogger(ShoppingCategoryController.class);
    ...
    private String displayCategory(final String friendlyUrl, final String ref, Model model, HttpServletRequest request, HttpServletResponse response, Locale locale) throws Exception {
        ...
        if (category == null) {
            ...
            return build404(store);

        }

        if (!category.isVisible()) {
            return buildHomePage(store);
        }
        ...
        String lineage = new StringBuilder().append(category.getLineage()).append(category.getId()).append(CATEGORY_LINEAGE_DELIMITER).toString();
        ...
        subCategoriesCacheKey
                ...
                .append("_")
                .append(SUBCATEGORIES_CACHE_KEY)
                .append("-")
        ...
        if (parentProxy != null) {
            request.setAttribute(LINK_CODE, parentProxy.getDescription().getFriendlyUrl());
        }


        /** template **/
        StringBuilder template = new StringBuilder().append(category).append(".").append(store.getStoreTemplate());
    ...
    private List<ReadableManufacturer> getManufacturersByProductAndCategory(MerchantStore store, Category category, List<Long> subCategoryIds, Language language) throws Exception {
        ...
        if (subCategoryIds != null && subCategoryIds.size() > 0) {
            ...
            manufacturersKey
                    .append(store.getId())
                    .append("_")
                    .append(MANUFACTURERS_BY_PRODUCTS_CACHE_KEY)
                    .append("-")
                    .append(language.getCode());
        ...
        for (Object[] counts : countProductsByCategories) {
            ...
            if (c.getParent() != null) {
                if (c.getParent().getId() == category.getId()) {
                    ...
                    String[] categoryPath = lin.split(CATEGORY_LINEAGE_DELIMITER);
                    for (int i = 0; i < categoryPath.length; i++) {
                        String sId = categoryPath[i];
                        if (!isBlank(sId)) {
                            Long count = countByCategories.get(parseLong(sId));
                            if (count != null) {
                                count = count + (Long) counts[1];
                                countByCategories.put(parseLong(sId), count);
                            }
                        }
                    }
                }
    ...
    public List<ReadableCategory> getCategories(@PathVariable final String language, @PathVariable final String store, Model model, HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        if (l == null) {
            l = languageService.getByCode(DEFAULT_LANGUAGE);
        }
    ...
    public ProductList getProducts(@PathVariable final String store, @PathVariable final String language, @PathVariable final String category, Model model, HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        try {
            ...
            if (lang == null) {
                lang = langs.get(DEFAULT_LANGUAGE);
            }
    ...
    public ProductList getProductsFilteredByType(@PathVariable int start, @PathVariable int max, @PathVariable String store, @PathVariable final String language, @PathVariable final String category, @PathVariable final String filterType, @PathVariable final String filterValue, Model model, HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        try {
            if (filterType.equals(BRAND.name())) {//the only one implemented so far
                QueryFilter filter = new QueryFilter();
                filter.setFilterType(BRAND);
                filter.setFilterId(parseLong(filterValue));
        ...
        try {
            ...
            if (filters != null) {
                for (QueryFilter filter : filters) {
                    if (filter.getFilterType().name().equals(BRAND.name())) {//the only filter implemented
            ...
            sort(productList.getProducts(), new Comparator<ReadableProduct>() {
            ...
            if (isNotEmpty(prices)) {
                BigDecimal minPrice = (BigDecimal) min(prices);
                BigDecimal maxPrice = (BigDecimal) max(prices);
