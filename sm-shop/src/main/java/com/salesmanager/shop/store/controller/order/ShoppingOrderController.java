...
import static com.salesmanager.core.model.shipping.ShippingQuote.*;
import static com.salesmanager.shop.admin.model.userpassword.UserReset.generateRandomString;
...
import static com.salesmanager.shop.store.controller.ControllerConstants.Tiles.Checkout;
import static com.salesmanager.shop.store.controller.ControllerConstants.Tiles.Checkout.checkout;
import static com.salesmanager.shop.store.controller.ControllerConstants.Tiles.Pages;
import static com.salesmanager.shop.store.controller.ControllerConstants.Tiles.Pages.timeout;
...
public class ShoppingOrderController extends AbstractController {
    ...
    public String displayCheckout(@CookieValue("cart") String cookie, Model model, HttpServletRequest request, HttpServletResponse response, Locale locale) throws Exception {
        ...
        order = super.getSessionAttribute(ORDER, request);
        ...
        super.setSessionAttribute(SHOPPING_CART, cart.getShoppingCartCode(), request);
        ...
        if (customer != null) {
            ...
            AnonymousCustomer anonymousCustomer = (AnonymousCustomer) request.getAttribute(ANONYMOUS_CUSTOMER);
        ...
        if (quote != null) {
            ...
            if (isBlank(shippingReturnCode) || shippingReturnCode.equals(NO_POSTAL_CODE)) {

                if (order.getShippingSummary() == null) {
                    ShippingSummary summary = orderFacade.getShippingSummary(quote, store, language);
                    order.setShippingSummary(summary);
                    request.getSession().setAttribute(SHIPPING_SUMMARY, summary);//TODO DTO
                }
                ...
                request.getSession().setAttribute(SHIPPING_OPTIONS, options);//TODO DTO
                ...
                if (!isEmpty(options)) {
                    ...
                    for (ShippingOption shipOption : options) {
                        ...
                        //option name
                        if (!isBlank(shipOption.getOptionCode())) {
            ...
            if (quote.getDeliveryAddress() != null) {
                ...
                super.setSessionAttribute(KEY_SESSION_ADDRESS, deliveryAddress, request);
            }
        ...
        if (quote != null && quote.getShippingReturnCode() != null && quote.getShippingReturnCode().equals(NO_SHIPPING_MODULE_CONFIGURED)) {
        ...
        if (quote != null && !isBlank(quote.getQuoteError())) {
            LOGGER.error("Shipping quote error " + quote.getQuoteError());
            model.addAttribute("errorMessages", quote.getQuoteError());
        }

        if (quote != null && quote.getShippingReturnCode() != null && quote.getShippingReturnCode().equals(NO_SHIPPING_TO_SELECTED_COUNTRY)) {
            LOGGER.error("Shipping quote error " + quote.getShippingReturnCode());
        ...
        /** template **/
        StringBuilder template = new StringBuilder().append(checkout).append(".").append(store.getStoreTemplate());
    ...
    public String commitPreAuthorizedOrder(Model model, HttpServletRequest request, HttpServletResponse response, Locale locale) throws Exception {
        ...
        MerchantStore store = (MerchantStore) request.getAttribute(MERCHANT_STORE);
        Language language = (Language) request.getAttribute("LANGUAGE");
        ShopOrder order = super.getSessionAttribute(ORDER, request);
        if (order == null) {
            StringBuilder template = new StringBuilder().append(timeout).append(".").append(store.getStoreTemplate());
            return template.toString();
        }

        @SuppressWarnings("unchecked")
        Map<String, Object> configs = (Map<String, Object>) request.getAttribute(REQUEST_CONFIGS);

        if (configs != null && configs.containsKey(DEBUG_MODE)) {
            Boolean debugMode = (Boolean) configs.get(DEBUG_MODE);
            if (debugMode) {
                try {
        ...
        try {
            ...
            OrderTotalSummary totalSummary = super.getSessionAttribute(ORDER_SUMMARY, request);

            if (totalSummary == null) {
                totalSummary = orderFacade.calculateOrderTotal(store, order, language);
                super.setSessionAttribute(ORDER_SUMMARY, totalSummary, request);
            }
            ...
            super.setSessionAttribute(ORDER_ID, orderModel.getId(), request);
    ...
    private Order commitOrder(ShopOrder order, HttpServletRequest request, Locale locale) throws Exception, ServiceException {
        ...
        Transaction initialTransaction = (Transaction) super.getSessionAttribute(INIT_TRANSACTION_KEY, request);
        ...
        super.setSessionAttribute(ORDER_ID, modelOrder.getId(), request);
        //set a unique token for confirmation
        super.setSessionAttribute(ORDER_ID_TOKEN, modelOrder.getId(), request);


        //get cart
        String cartCode = super.getSessionAttribute(SHOPPING_CART, request);
        ...
        super.removeAttribute(ORDER, request);
        super.removeAttribute(ORDER_SUMMARY, request);
        super.removeAttribute(INIT_TRANSACTION_KEY, request);
        super.removeAttribute(SHIPPING_OPTIONS, request);
        super.removeAttribute(SHIPPING_SUMMARY, request);
        super.removeAttribute(SHOPPING_CART, request);
        ...
        try {
            ...
            if (isNotEmpty(orderProductDownloads)) {
                ...
                if (auth != null &&
                    ...
                    super.setSessionAttribute(CUSTOMER, modelCustomer, request);
                }
        ...
        try {
            ...
            if (order.getSelectedShippingOption() != null) {
                ...
                if (summary == null) {
                    ...
                    request.getSession().setAttribute(SHIPPING_SUMMARY, options);
                }
                ...
                if (!isEmpty(options)) {
                    ...
                    for (ShippingOption shipOption : options) {
                        if (!isBlank(shipOption.getOptionId()) && shipOption.getOptionId().equals(selectedOption.getOptionId())) {
            ...
            if (se.getExceptionType() == EXCEPTION_VALIDATION) {
                if (!isBlank(se.getMessageCode())) {
            ...
            } else if (se.getExceptionType() == EXCEPTION_PAYMENT_DECLINED) {
     ...
     */
    @SuppressWarnings("unchecked")
    @RequestMapping(value = {"/shippingQuotes.json"}, method = POST)
    public @ResponseBody
...
/*		if(configs!=null && configs.containsKey(Constants.DEBUG_MODE)) {
        ...
        try {
            ...
            if (quote != null) {
                String shippingReturnCode = quote.getShippingReturnCode();
                if (isNotEmpty(quote.getShippingOptions()) || NO_POSTAL_CODE.equals(shippingReturnCode)) {
...
/*					if(quote.getQuoteInformations() != null && quote.getQuoteInformations().size() >0) {
                    ...
                    if (configs != null && configs.containsKey(DEBUG_MODE)) {
                        ...
                        if (debugMode) {
                            ...
                            try {
                                ...
                                out.println("Readable order -> shoppingCartCode[ " + shoppingCartCode + "] -> " + jsonInString);
                            } catch (Exception de) {
                                LOGGER.error(de.getMessage());
                            }
                ...
                if (quote.getShippingReturnCode() != null && quote.getShippingReturnCode().equals(NO_SHIPPING_MODULE_CONFIGURED)) {
                    LOGGER.error("Shipping quote error " + quote.getShippingReturnCode());
                    readableOrder.setErrorMessage(messages.getMessage("message.noshipping", locale));
                }

                if (quote.getShippingReturnCode() != null && quote.getShippingReturnCode().equals(NO_SHIPPING_TO_SELECTED_COUNTRY)) {
                    if (isEmpty(quote.getShippingOptions())) {//only if there are no other options
                        LOGGER.error("Shipping quote error " + quote.getShippingReturnCode());
                        readableOrder.setErrorMessage(messages.getMessage("message.noshipping", locale));
                    }
                }
                ...
                if (!isBlank(quote.getQuoteError())) {
                    LOGGER.error("Shipping quote error " + quote.getQuoteError());
     ...
     * @throws Exception
     */
    @RequestMapping(value = {"/calculateOrderTotal.json"}, method = POST)
    public @ResponseBody
    ReadableShopOrder calculateOrderTotal(@ModelAttribute(value = "order") ShopOrder order, HttpServletRequest request, HttpServletResponse response, Locale locale) throws Exception {
        ...
        try {
            ...
            ReadableDelivery readableDelivery = super.getSessionAttribute(KEY_SESSION_ADDRESS, request);
