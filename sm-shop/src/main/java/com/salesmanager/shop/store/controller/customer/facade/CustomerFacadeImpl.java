...
import static com.salesmanager.core.model.user.GroupType.CUSTOMER;
import static com.salesmanager.shop.admin.model.userpassword.UserReset.generateRandomString;
import static com.salesmanager.shop.constants.Constants.GROUP_CUSTOMER;
import static com.salesmanager.shop.constants.Constants.PERMISSION_CUSTOMER_AUTHENTICATED;
...
public class CustomerFacadeImpl implements CustomerFacade {
    ...
    private static final Logger LOG = getLogger(CustomerFacadeImpl.class);
    ...
    */
    ...
    public ShoppingCart mergeCart(final Customer customerModel, final String sessionShoppingCartId, final MerchantStore store, final Language language)
        ...
        if (customerModel != null) {
            ...
            if (isNotBlank(sessionShoppingCartId)) {
    ...
    public boolean checkIfUserExists(final String userName, final MerchantStore store)
            throws Exception {
        if (isNotBlank(userName) && store != null) {
    ...
    public Customer getCustomerModel(final PersistableCustomer customer, final MerchantStore merchantStore, Language language) throws Exception {
        ...
        if (isBlank(customerModel.getPassword()) && !isBlank(customer.getClearPassword())) {
            customerModel.setPassword(customer.getClearPassword());
        }
        //set groups
        if (!isBlank(customerModel.getPassword()) && !isBlank(customerModel.getNick())) {
    ...
    public void setCustomerModelDefaultProperties(Customer customer,
        ...
        notNull(customer, "Customer object cannot be null");
        if (customer.getId() == null || customer.getId() == 0) {
            if (isBlank(customer.getNick())) {
        ...
        if (isEmpty(customer.getGroups())) {
            List<Group> groups = groupService.listGroup(CUSTOMER);
            for (Group group : groups) {
                if (group.getGroupName().equals(GROUP_CUSTOMER)) {
    ...
    public void authenticate(Customer customer, String userName, String password) throws Exception {
        ...
        GrantedAuthority role = new SimpleGrantedAuthority(ROLE_PREFIX + PERMISSION_CUSTOMER_AUTHENTICATED);//required to login
    ...
    private Customer populate(PersistableCustomer customer, MerchantStore store) throws Exception {
        ...
        Locale customerLocale = getLocale(cust.getDefaultLanguage());
        ...
        @SuppressWarnings("deprecation")
        String encodedPassword = passwordEncoder.encode(password);
        if (!isBlank(customer.getEncodedPassword())) {
    ...
    public List<ReadableCustomerReview> getAllCustomerReviewsByReviewed(Customer customer, MerchantStore store,
                                                                        Language language) throws Exception {
        notNull(customer, "Reviewed customer cannot be null");
