...
import static com.salesmanager.core.model.catalog.product.relationship.ProductRelationshipType.RELATED_ITEM;
...
public class ShopProductController {
    ...
    private static final Logger LOG = getLogger(ShopProductController.class);
    ...
    public String display(final String reference, final String friendlyUrl, Model model, HttpServletRequest request, HttpServletResponse response, Locale locale) throws Exception {
        ...
        if (!isEmpty(attributes)) {
            ...
            for (ProductAttribute attribute : attributes) {
                ...
                if (!isBlank(attribute.getProductOptionValue().getProductOptionValueImage())) {
        ...
        if (selectableOptions != null) {
            ...
            for (Attribute attr : optionsList) {
                sort(attr.getValues(), new Comparator<AttributeValue>() {
        ...
        /** template **/
        StringBuilder template = new StringBuilder().append(product).append(".").append(store.getStoreTemplate());

        return template.toString();
    }

    @RequestMapping(value = {"/{productId}/calculatePrice.json"}, method = POST)
    public @ResponseBody
    ReadableProductPrice calculatePrice(@RequestParam(value = "attributeIds[]") Long[] attributeIds, @PathVariable final Long productId, final HttpServletRequest request, final HttpServletResponse response, final Locale locale) throws Exception {
        ...
        List<Long> ids = new ArrayList<Long>(asList(attributeIds));
    ...
    private List<ReadableProduct> relatedItems(MerchantStore store, Product product, Language language) throws Exception {
        ...
        List<ProductRelationship> relatedItems = productRelationshipService.getByType(store, product, RELATED_ITEM);
