...
import static com.salesmanager.shop.store.controller.ControllerConstants.Tiles.Customer.register;
...
public class CustomerRegistrationController extends AbstractController {
    ...
    private static final Logger LOGGER = getLogger(CustomerRegistrationController.class);
    ...
    @RequestMapping(value = "/registration.html", method = GET)
    public String displayRegistration(final Model model, final HttpServletRequest request, final HttpServletResponse response) throws Exception {
        ...
        /** template **/
        StringBuilder template = new StringBuilder().append(register).append(".").append(store.getStoreTemplate());
    ...
    public String registerCustomer(@Valid
        ...
        if (isNotBlank(customer.getUserName())) {
        ...
        if (isNotBlank(customer.getPassword()) && isNotBlank(customer.getCheckPassword())) {
        ...
        if (bindingResult.hasErrors()) {
            ...
            StringBuilder template =
                    new StringBuilder().append(register).append(".").append(merchantStore.getStoreTemplate());
            return template.toString();
       ...
       /* catch ( CustomerRegistrationException cre )
        ...
        try {
            ...
            Cookie cookie = new Cookie(COOKIE_NAME_USER, cookieValue.toString());
            cookie.setMaxAge(60 * 24 * 3600);
            cookie.setPath(SLASH);
            response.addCookie(cookie);


            String sessionShoppingCartCode = (String) request.getSession().getAttribute(SHOPPING_CART);
            if (!isBlank(sessionShoppingCartCode)) {
                ...
                if (shoppingCartData != null) {
                    request.getSession().setAttribute(SHOPPING_CART, shoppingCartData.getCode());
                }

                //set username in the cookie
                Cookie c1 = new Cookie(COOKIE_NAME_CART, shoppingCartData.getCode());
