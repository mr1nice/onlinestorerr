...
public class CustomerLoginController extends AbstractController {
    ...
    private static final Logger LOG = getLogger(CustomerLoginController.class);
    ...
    private AjaxResponse logon(String userName, String password, String storeCode, HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        try {
            ...
            if (customerModel == null) {
                jsonObject.setStatus(RESPONSE_STATUS_FAIURE);
                return jsonObject;
            }
            ...
            super.setSessionAttribute(CUSTOMER, customerModel, request);
            jsonObject.setStatus(RESPONSE_STATUS_SUCCESS);
            jsonObject.addEntry(RESPONSE_KEY_USERNAME, customerModel.getNick());
            ...
            if (!isBlank(sessionShoppingCartCode)) {
                ...
                if (shoppingCart != null) {
                    ...
                    if (shoppingCartData != null) {
                        jsonObject.addEntry(SHOPPING_CART, shoppingCartData.getCode());
                        request.getSession().setAttribute(SHOPPING_CART, shoppingCartData.getCode());

                        //set cart in the cookie
                        Cookie c = new Cookie(COOKIE_NAME_CART, shoppingCartData.getCode());
                        ...
                        //DELETE COOKIE
                        Cookie c = new Cookie(COOKIE_NAME_CART, "");
                        c.setMaxAge(0);
                        c.setPath(SLASH);
                        response.addCookie(c);
                    }
                ...
                if (cartModel != null) {
                    jsonObject.addEntry(SHOPPING_CART, cartModel.getShoppingCartCode());
                    request.getSession().setAttribute(SHOPPING_CART, cartModel.getShoppingCartCode());

                    Cookie c = new Cookie(COOKIE_NAME_CART, cartModel.getShoppingCartCode());
                    c.setMaxAge(60 * 24 * 3600);
                    c.setPath(SLASH);
                    response.addCookie(c);
            ...
            Cookie c = new Cookie(COOKIE_NAME_USER, cookieValue.toString());
            c.setMaxAge(60 * 24 * 3600);
            c.setPath(SLASH);
            response.addCookie(c);
    ...
    @RequestMapping(value = "/authenticate.html", method = GET)
    public @ResponseBody
