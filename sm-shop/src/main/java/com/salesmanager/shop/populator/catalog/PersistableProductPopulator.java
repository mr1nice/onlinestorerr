...
import com.salesmanager.shop.model.catalog.product.attribute.ProductAttributeEntity;
...
import static com.salesmanager.core.model.catalog.product.price.ProductPriceDescription.DEFAULT_PRICE_DESCRIPTION;
import static com.salesmanager.shop.model.catalog.product.ProductPrice.DEFAULT_PRICE_CODE;
...
public class PersistableProductPopulator extends
	...
	public Product populate(PersistableProduct source,
		...
		notNull(customerService, "Requires to set CustomerService");
		...
		try {
			...
			if (!isBlank(source.getDateAvailable())) {
				target.setDateAvailable(getDate(source.getDateAvailable()));
			}

			if (source.getManufacturer() != null) {

				Manufacturer manuf = null;
				if (!isBlank(source.getManufacturer().getCode())) {
					manuf = manufacturerService.getByCode(store, source.getManufacturer().getCode());
				} else {
					notNull(source.getManufacturer().getId(), "Requires to set manufacturer id");
			...
			if (!isEmpty(source.getDescriptions())) {
			...
			if (isNotEmpty(source.getProductPrices())) {
				...
				for (ProductPriceEntity priceEntity : source.getProductPrices()) {
					...
					if (priceEntity.getDiscountStartDate() != null) {
						Date startDate = getDate(priceEntity.getDiscountStartDate());
						price.setProductPriceSpecialStartDate(startDate);
					}
					if (priceEntity.getDiscountEndDate() != null) {
						Date endDate = getDate(priceEntity.getDiscountEndDate());
					...
					for (Language lang : languages) {
						...
						ppd.setName(DEFAULT_PRICE_DESCRIPTION);
				...
				price.setCode(DEFAULT_PRICE_CODE);
				price.setProductAvailability(productAvailability);
				productAvailability.getPrices().add(price);
				target.getAvailabilities().add(productAvailability);
				for (Language lang : languages) {
			...
			if (source.getAttributes() != null) {
				for (ProductAttributeEntity attr : source.getAttributes()) {

					ProductOption productOption = null;

					if (!isBlank(attr.getOption().getCode())) {
						productOption = productOptionService.getByCode(store, attr.getOption().getCode());
					} else {
						notNull(attr.getOption().getId(), "Product option id is null");
					...
					if (!isBlank(attr.getOptionValue().getCode())) {
			...
			if (!isEmpty(source.getCategories())) {
				for (com.salesmanager.shop.model.catalog.category.Category categ : source.getCategories()) {
					...
					Category c = null;
					if (!isBlank(categ.getCode())) {
						c = categoryService.getByCode(store, categ.getCode());
					} else {
						notNull(categ.getId(), "Category id nust not be null");
