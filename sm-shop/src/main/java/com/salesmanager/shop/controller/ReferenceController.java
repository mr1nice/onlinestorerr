...
import static com.salesmanager.shop.utils.DateUtil.formatYear;
...
public class ReferenceController {
    ...
    private static final Logger LOGGER = getLogger(ReferenceController.class);
    ...
    @RequestMapping(value = {"/admin/reference/provinces.html", "/shop/reference/provinces.html"}, method = POST)
    public @ResponseBody
    ...
    @SuppressWarnings("unchecked")
    @RequestMapping(value = {"/shop/reference/creditCardDates.html"}, method = GET)
    public @ResponseBody
    ResponseEntity<String> getCreditCardDates(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            years = (List<String>) cache.getFromCache(CREDIT_CARD_YEARS_CACHE_KEY);
            ...
            if (years == null) {
                ...
                for (int i = 0; i < 10; i++) {
                    Calendar localCalendar = getInstance(getDefault());
                    localCalendar.add(YEAR, i);
                    String dt = formatYear(localCalendar.getTime());
                    years.add(dt);
                }
                //up to year + 10

                cache.putInCache(years, CREDIT_CARD_YEARS_CACHE_KEY);
        ...
        final HttpHeaders httpHeaders = new HttpHeaders();
        httpHeaders.setContentType(APPLICATION_JSON_UTF8);
        return new ResponseEntity<String>(serialized, httpHeaders, OK);

    }


    @SuppressWarnings("unchecked")
    @RequestMapping(value = {"/shop/reference/monthsOfYear.html"}, method = GET)
    public @ResponseBody
    ResponseEntity<String> getMonthsOfYear(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            days = (List<String>) cache.getFromCache(MONTHS_OF_YEAR_CACHE_KEY);
            if (days == null) {

                days = new ArrayList<String>();
                for (int i = 1; i < 13; i++) {
                    days.add(format("%02d", i));
                }

                cache.putInCache(days, MONTHS_OF_YEAR_CACHE_KEY);
