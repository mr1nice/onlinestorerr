...
import static com.salesmanager.core.model.catalog.product.price.ProductPriceType.ONE_TIME;
import static com.salesmanager.shop.admin.controller.ControllerConstants.Tiles;
import static com.salesmanager.shop.admin.controller.ControllerConstants.Tiles.Product.productPrice;
import static com.salesmanager.shop.admin.controller.ControllerConstants.Tiles.Product.productPrices;
...
public class ProductPriceController {
	...
	private String displayProductPrice(Product product, Long productPriceId, Model model, HttpServletRequest request, HttpServletResponse response) throws Exception {
                    ...
                    for (ProductPrice price : prices) {
                        if (price.getId().longValue() == productPriceId.longValue()) {
                            ...
                            if (price.getProductPriceSpecialStartDate() != null) {
                                pprice.setProductPriceSpecialStartDate(formatDate(price.getProductPriceSpecialStartDate()));
                            }
                            if (price.getProductPriceSpecialEndDate() != null) {
                                pprice.setProductPriceSpecialEndDate(formatDate(price.getProductPriceSpecialEndDate()));
                            }
        ...
        if (productPrice == null) {
            ...
            productPrice.setProductPriceType(ONE_TIME);
        }
	...
	public String saveProductPrice(@Valid @ModelAttribute("price") com.salesmanager.shop.admin.model.catalog.ProductPrice price, BindingResult result, Model model, HttpServletRequest request, Locale locale) throws Exception {
        ...
        if (!isBlank(price.getSpecialPriceText())) {
            try {
        ...
        if (!isBlank(price.getProductPriceSpecialStartDate())) {
            try {
                Date startDate = getDate(price.getProductPriceSpecialStartDate());
        ...
        if (!isBlank(price.getProductPriceSpecialEndDate())) {
            try {
                Date endDate = getDate(price.getProductPriceSpecialEndDate());
