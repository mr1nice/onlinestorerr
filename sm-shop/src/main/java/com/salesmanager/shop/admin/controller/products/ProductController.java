...
public class ProductController {
	...
	private String displayProduct(Long productId, Model model, HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        if (productId != null && productId != 0) {
            ...
            product.setDateAvailable(formatDate(dbProduct.getDateAvailable()));
            ...
            product.setDescriptions(descriptions);
            product.setDateAvailable(formatDate(new Date()));
	...
	public String saveProduct(@Valid @ModelAttribute("product") com.salesmanager.shop.admin.model.catalog.Product  product, BindingResult result, Model model, HttpServletRequest request, Locale locale) throws Exception {
        ...
        Date date = new Date();
        if (!isBlank(product.getDateAvailable())) {
            try {
                date = getDate(product.getDateAvailable());
                product.getAvailability().setProductDateAvailable(date);
                product.setDateAvailable(formatDate(date));
            } catch (Exception e) {
        ...
        if (product.getImage() != null && !product.getImage().isEmpty()) {
            ...
            try {
                ...
                BufferedImage image = read(product.getImage().getInputStream());


                if (!isBlank(maxHeight)) {

                    int maxImageHeight = parseInt(maxHeight);
                ...
                if (!isBlank(maxWidth)) {
                    ...
                    int maxImageWidth = parseInt(maxWidth);
                ...
                if (!isBlank(maxSize)) {
                    ...
                    int maxImageSize = parseInt(maxSize);
    ...
    public String duplicateProduct(@ModelAttribute("productId") Long id, BindingResult result, Model model, HttpServletRequest request, Locale locale) throws Exception {
        ...
        newProduct.setSku(randomUUID().toString().replace("-", ""));
    ...
    ResponseEntity<String> pageProductCategories(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            productId = parseLong(sProductId);
        } catch (Exception e) {
            resp.setStatus(RESPONSE_STATUS_FAIURE);
