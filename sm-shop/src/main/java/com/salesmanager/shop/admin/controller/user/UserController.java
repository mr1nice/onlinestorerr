...
import static com.salesmanager.shop.admin.controller.ControllerConstants.Tiles.User.*;
...
import static com.salesmanager.shop.utils.UserUtils.userInGroup;
...
public class UserController {
    ...
    ResponseEntity<String> pageUsers(HttpServletRequest request,
        ...
        try {
            ...
            if (userInGroup(currentUser, GROUP_SUPERADMIN)) {
            ...
            for (User user : users) {
                ...
                if (!userInGroup(user, GROUP_SUPERADMIN)) {
        ...
        return new ResponseEntity<String>(returnString, OK);
    }
	...
	public String changePassword(@ModelAttribute("password") Password password, BindingResult result, Model model, HttpServletRequest request, HttpServletResponse response, Locale locale) throws Exception {
        ...
        if (isBlank(password.getPassword())) {
        ...
        if (isBlank(password.getNewPassword())) {
        ...
        if (isBlank(password.getRepeatPassword())) {
	...
	private void populateUserObjects(User user, MerchantStore store, Model model, Locale locale) throws Exception {
        ...
        List<Group> userGroups = groupService.listGroup(ADMIN);
        for (Group group : userGroups) {
            if (!group.getGroupName().equals(GROUP_SUPERADMIN)) {
                groups.add(group);
            }
        }
	...
	public String saveUser(@Valid @ModelAttribute("user") User user, BindingResult result, Model model, HttpServletRequest request, Locale locale) throws Exception {
        ...
        if (user.getId() == null || user.getId().longValue() == 0) {
            ...
            try {
                ...
                templateTokens.put(EMAIL_ADMIN_PASSWORD_LABEL, messages.getMessage("label.generic.password", userLocale));
                templateTokens.put(EMAIL_ADMIN_PASSWORD, decodedPassword);
	...
	public @ResponseBody ResponseEntity<String> removeUser(HttpServletRequest request, Locale locale) throws Exception {
        ...
        try {
            ...
            if (userInGroup(remoteUser, GROUP_ADMIN) || userInGroup(remoteUser, GROUP_SUPERADMIN)) {
                isAdmin = true;
            }
            ...
            if (dbUser != null) {
                ...
                if (dbUser.getAnswer1().equals(answer1.trim()) && dbUser.getAnswer2().equals(answer2.trim()) && dbUser.getAnswer3().equals(answer3.trim())) {
                    ...
                    try {
                        ...
                        Map<String, String> templateTokens = emailUtils.createEmailObjectsMap(request.getContextPath(), store, messages, userLocale);
                        templateTokens.put(EMAIL_RESET_PASSWORD_TXT, messages.getMessage("email.user.resetpassword.text", userLocale));
                        templateTokens.put(EMAIL_CONTACT_OWNER, messages.getMessage("email.contactowner", storeEmail, userLocale));
                        templateTokens.put(EMAIL_PASSWORD_LABEL, messages.getMessage("label.generic.password", userLocale));
                        templateTokens.put(EMAIL_USER_PASSWORD, tempPass);
