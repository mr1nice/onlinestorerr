...
import static com.salesmanager.shop.constants.Constants.GROUP_SUPERADMIN;
...
public class MerchantStoreController {
	...
	private String displayMerchantStore(MerchantStore store, Model model, HttpServletRequest request, HttpServletResponse response, Locale locale) throws Exception {
		...
		if (dt != null) {
			store.setDateBusinessSince(formatDate(dt));
		} else {
			store.setDateBusinessSince(formatDate(new Date()));
		}
	...
	public String saveMerchantStore(@Valid @ModelAttribute("store") MerchantStore store, BindingResult result, Model model, HttpServletRequest request, HttpServletResponse response, Locale locale) throws Exception {
		...
		setMenu(model, request);
		MerchantStore sessionStore = (MerchantStore) request.getAttribute(ADMIN_STORE);
		...
		Date date = new Date();
		if (!isBlank(store.getDateBusinessSince())) {
			try {
				date = getDate(store.getDateBusinessSince());
		...
		if ((zonesList == null || zonesList.size() == 0) && isBlank(store.getStorestateprovince())) {
		...
		Locale storeLocale = getLocale(defaultLanguage);
		...
		if (!store.getCode().equals(sessionStore.getCode())) {
			...
			try {
				...
				templateTokens.put(EMAIL_NEW_STORE_TEXT, messages.getMessage("email.newstore.text", storeLocale));
				templateTokens.put(EMAIL_STORE_NAME, messages.getMessage("email.newstore.name", new String[]{store.getStorename()}, storeLocale));
				templateTokens.put(EMAIL_ADMIN_STORE_INFO_LABEL, messages.getMessage("email.newstore.info", storeLocale));

				templateTokens.put(EMAIL_ADMIN_URL_LABEL, messages.getMessage("label.adminurl", storeLocale));
		...
		request.getSession().setAttribute(ADMIN_STORE, sessionStore);
	...
	public @ResponseBody ResponseEntity<String> removeMerchantStore(HttpServletRequest request, Locale locale) throws Exception {
		...
		try {
			...
			Integer storeId = parseInt(sMerchantStoreId);
