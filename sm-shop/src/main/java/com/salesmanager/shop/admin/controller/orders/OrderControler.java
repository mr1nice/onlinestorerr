...
import static com.salesmanager.shop.admin.controller.ControllerConstants.Tiles.Order.ordersEdit;
...
import static com.salesmanager.shop.utils.DateUtil.getDate;
...
public class OrderControler {
    ...
    private static final Logger LOGGER = getLogger(OrderControler.class);
    ...
    @RequestMapping(value = "/admin/orders/editOrder.html", method = GET)
    ...
    private String displayOrder(Long orderId, Model model, HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        if (orderId != null && orderId != 0) {        //edit mode
            ...
            if (dbOrder.getDatePurchased() != null) {
                order.setDatePurchased(formatDate(dbOrder.getDatePurchased()));
            }
            ...
            //get capturable
            if (dbOrder.getPaymentType().name() != MONEYORDER.name()) {
    ...
    @PreAuthorize("hasRole('ORDER')")
    @RequestMapping(value = "/admin/orders/save.html", method = POST)
    public String saveOrder(@Valid @ModelAttribute("order") com.salesmanager.shop.admin.model.orders.Order entityOrder, BindingResult result, Model model, HttpServletRequest request, Locale locale) throws Exception {
        ...
        Date date = new Date();
        if (!isBlank(entityOrder.getDatePurchased())) {
            try {
                date = getDate(entityOrder.getDatePurchased());
            } catch (Exception e) {
        ...
        if (!isBlank(entityOrder.getOrder().getCustomerEmailAddress())) {
            Matcher matcher = pattern.matcher(entityOrder.getOrder().getCustomerEmailAddress());
        ...
        if (isBlank(entityOrder.getOrder().getBilling().getFirstName())) {
        ...
        if (entityOrder.getOrder().getBilling().getZone() == null) {
            if (isBlank(entityOrder.getOrder().getBilling().getState())) {
                ObjectError error = new ObjectError("billingState", messages.getMessage("NotEmpty.order.billingState", locale));
                result.addError(error);
            }
        }

        if (isBlank(entityOrder.getOrder().getBilling().getPostalCode())) {
        ...
        //get capturable
        if (newOrder.getPaymentType().name() != MONEYORDER.name()) {
        ...
        if (!isBlank(entityOrder.getOrderHistoryComment())) {
        ...
        if (isBlank(entityOrder.getOrderHistoryComment())) {
            ...
            try {
                ...
                Locale customerLocale = getLocale(lang);
                ...
                templateTokens.put(EMAIL_CUSTOMER_NAME, customerName.toString());
                templateTokens.put(EMAIL_TEXT_ORDER_NUMBER, messages.getMessage("email.order.confirmation", new String[]{valueOf(newOrder.getId())}, customerLocale));
                templateTokens.put(EMAIL_TEXT_DATE_ORDERED, messages.getMessage("email.order.ordered", new String[]{entityOrder.getDatePurchased()}, customerLocale));
                templateTokens.put(EMAIL_TEXT_STATUS_COMMENTS, messages.getMessage("email.order.comments", new String[]{entityOrder.getOrderHistoryComment()}, customerLocale));
                templateTokens.put(EMAIL_TEXT_DATE_UPDATED, messages.getMessage("email.order.updated", new String[]{formatDate(new Date())}, customerLocale));


                Email email = new Email();
                email.setFrom(store.getStorename());
                email.setFromEmail(store.getStoreEmailAddress());
                email.setSubject(messages.getMessage("email.order.status.title", new String[]{valueOf(newOrder.getId())}, customerLocale));
