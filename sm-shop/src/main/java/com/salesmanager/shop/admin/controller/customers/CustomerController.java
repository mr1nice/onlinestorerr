...
public class CustomerController {
	...
	public String displayCustomer(Long id, Model model, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		List<Group> userGroups = groupService.listGroup(CUSTOMER);
				...
				if (!isEmpty(customerAttributes)) {
					for (CustomerAttribute customerAttribute : customerAttributes) {
						if (customerAttribute.getCustomerOption().getId().longValue() == customerOption.getId()) {
							...
							if (customerOption.getType().equalsIgnoreCase(Text.name())) {
	...
	public String saveCustomer(@Valid @ModelAttribute("customer") Customer customer, BindingResult result, Model model, HttpServletRequest request, Locale locale) throws Exception {
		...
		if (isBlank(customer.getBilling().getLastName())) {
		...
		if (customer.getId() != null && customer.getId().longValue() > 0) {
			...
			MerchantStore merchantStore = (MerchantStore) request.getAttribute(ADMIN_STORE);
	...
	ResponseEntity<String>  pageCustomers(HttpServletRequest request,HttpServletResponse response) {
		...
		try {
			...
			int startRow = parseInt(request.getParameter("_startRow"));
			int endRow = parseInt(request.getParameter("_endRow"));
	...
	ResponseEntity<String> resetPassword(HttpServletRequest request,HttpServletResponse response) {
		...
		try {
			...
			Locale customerLocale = getLocale(userLanguage);
			...
			try {
				...
				templateTokens.put(EMAIL_RESET_PASSWORD_TXT, messages.getMessage("email.customer.resetpassword.text", customerLocale));
				templateTokens.put(EMAIL_CONTACT_OWNER, messages.getMessage("email.contactowner", storeEmail, customerLocale));
				templateTokens.put(EMAIL_PASSWORD_LABEL, messages.getMessage("label.generic.password", customerLocale));
				templateTokens.put(EMAIL_CUSTOMER_PASSWORD, password);
