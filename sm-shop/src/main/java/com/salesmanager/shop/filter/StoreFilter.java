...
import static com.salesmanager.core.model.content.ContentType.*;
...
import static com.salesmanager.shop.model.shop.BreadcrumbItemType.PAGE;
import static com.salesmanager.shop.utils.GeoLocationUtils.getClientIpAddress;
...
public class StoreFilter extends HandlerInterceptorAdapter {
    ...
    private static final Logger LOGGER = getLogger(StoreFilter.class);
    ...
    public boolean preHandle(
        ...
        try {
            ...
            request.removeAttribute(LINK_CODE);
            ...
            AnonymousCustomer anonymousCustomer = (AnonymousCustomer) request.getSession().getAttribute(ANONYMOUS_CUSTOMER);
            if (anonymousCustomer == null) {
                ...
                request.getSession().setAttribute(ANONYMOUS_CUSTOMER, anonymousCustomer);
            } else {
            ...
            Map<String, ContentDescription> contents = (Map<String, ContentDescription>) request.getAttribute(REQUEST_CONTENT_OBJECTS);
            ...
            if (contents != null) {
                ...
                ContentDescription contentDescription = contents.get(CONTENT_LANDING_PAGE);
    ...
    private void getContentPageNames(MerchantStore store, Language language, HttpServletRequest request) throws Exception {
        ...
        if (contents != null && contents.size() > 0) {
            ...
            if (descriptions != null) {
                request.setAttribute(REQUEST_CONTENT_PAGE_OBJECTS, descriptions);
            }
        }
    }
    ...
    private void getContentObjects(MerchantStore store, Language language, HttpServletRequest request) throws Exception {
        ...
        if (contents != null && contents.size() > 0) {
            ...
            if (!isEmpty(contentByStore)) {
                ...
                request.setAttribute(REQUEST_CONTENT_OBJECTS, contentMap);
            }
    ...
    private void setTopCategories(MerchantStore store, Language language, HttpServletRequest request) throws Exception {
        ...
        if (loadedCategories != null) {
            request.setAttribute(REQUEST_TOP_CATEGORIES, loadedCategories);
        }
    ...
    private Map<String, List<Content>> getContent(MerchantStore store, Language language) throws Exception {
        ...
        //Get boxes and sections from the database
        List<ContentType> contentTypes = new ArrayList<ContentType>();
        contentTypes.add(BOX);
        contentTypes.add(SECTION);
    ...
    private Map<String, Object> getConfigurations(MerchantStore store) {
        ...
        try {
            ...
            List<MerchantConfiguration> merchantConfiguration = merchantConfigurationService.listByType(CONFIG, store);

            if (isEmpty(merchantConfiguration)) {
                return configs;
            }
            ...
            if (breadCrumb == null) {
                ...
                if (language.getCode().equals(breadCrumb.getLanguage().getCode())) {
                    ...
                    for (BreadcrumbItem item : breadCrumb.getBreadCrumbs()) {
                        ...
                        } else if (item.getItemType().name().equals(CATEGORY)) {
